"""
GroundingDINO + SAM segmentation backend.
Open-vocabulary detection gives boxes, SAM turns them into masks.
"""
...
class GroundedSegmenter:
    """Per-object masks from text prompts via GroundingDINO boxes + SAM."""
    ...
    def detect_objects(
        ...
        """Returns boxes [N,4] xyxy, scores [N], label strings [N]."""
    ...
    def segment_from_boxes(
        ...
        """SAM masks [N,H,W] for the given boxes [N,4] xyxy."""
        if len(boxes) == 0:
        ...
        for prompt in object_prompts:
            ...
            for i, (mask, label) in enumerate(zip(masks, detected_labels)):
                ...
                if clean_label in label_counts:
                    ...
                    label_counts[clean_label] = 1
                    # _1 suffix only if more instances of the same label follow
